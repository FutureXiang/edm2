...
class EDM(nn.Module):
    def __init__(self, nn_model,
        ...
        self.reweight_mlp = self.nn_model.get_reweighting()
    ...
    def forward(self, x, use_amp=False):
        ...
        u = self.nn_model.forward_reweighting(self.reweight_mlp, sigma)
        loss_4shape = weight * ((x - self.D_x(x_noised, sigma, use_amp)) ** 2) / u.exp() + u
    ...
    def D_x(self, x_noised, sigma, use_amp, ret_activation=False):
        ...
        with autocast(enabled=use_amp):
            F_x = self.nn_model(c_in * x_noised, c_noise.flatten(), ret_activation)
